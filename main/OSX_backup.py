...
class Model(nn.Module):
    def __init__(self, encoder, body_position_net, body_rotation_net, box_net, hand_position_net, hand_roi_net, hand_decoder,
        ...
        #self.smplh_layer = copy.deepcopy(smpl_h.layer['neutral']).cuda()
        self.smpl = smpl
    ...
    def get_coord_hshi(self, root_pose, body_pose, lhand_pose, rhand_pose, jaw_pose, shape, expr, cam_trans, mode):
        ...
        #        / (joint_cam[:, :, 2] + cam_trans[:, None, 2] + 1e-4) * cfg.focal[0] + cfg.princpt[0]
        #    y = (joint_cam[:, :, 1] + cam_trans[:, None, 1]) \
        #        / (joint_cam[:, :, 2] + cam_trans[:, None, 2] + 1e-4) * cfg.focal[1] + cfg.princpt[1]
    ...
    def forward(self, inputs, targets, meta_info, mode):
        ...
        shape_token, cam_token, expr_token, jaw_pose_token, hand_token, body_pose_token = \
            task_tokens[:, 0].detach(), task_tokens[:, 1], task_tokens[:, 2], task_tokens[:, 3], task_tokens[:, 4:6], task_tokens[:, 6:]
        #pdb.set_trace()
        # 2. Body Regressor
        ...
        silhouette, joint_proj = self.camera_screen(mesh_cam, torch.from_numpy(self.smpl.face.astype('int')).unsqueeze(0), joint_proj)


        

        #img_input = np.ascontiguousarray(body_img[-1, ...].permute([1,2,0]).detach().cpu().numpy() * 255, dtype=np.uint8)
        ...
        if mode == 'train':
            ...
            # trans = torch.stack(trans)[:, None, :]
            # coord = coord + trans  # global translation alignment
            # joint_proj = torch.cat((joint_proj[:, :self.smpl.joint_part['face'][0], :], coord,
            #                         joint_proj[:, self.smpl.joint_part['face'][-1] + 1:, :]), 1)
            ...
            if cfg.debug:
                ...
                rendered = torch.multiply(body_img[-1,...].permute([1,2,0]), 1 - silhouette[-1, :, :, 3].unsqueeze(-1)) + silhouette[-1, :, :, 3].unsqueeze(-1)
                #rendered = np.ascontiguousarray(rendered.cpu().numpy()*255)
                #rendered = vis_keypoints(np.ascontiguousarray(rendered.cpu().numpy()*255), proj_points[-1,...].cpu().numpy())

                kpt_gt = targets['joint_img'][-1, ...].cpu().numpy().copy()
                kpt_gt[:, 0] = kpt_gt[:, 0] / cfg.output_hm_shape[2] * cfg.input_body_shape[1]
                kpt_gt[:, 1] = kpt_gt[:, 1] / cfg.output_hm_shape[1] * cfg.input_body_shape[0]

                img_input = np.ascontiguousarray(body_img[-1, ...].permute([1,2,0]).detach().cpu().numpy() * 255, dtype=np.uint8)

                #for i in range(kpt_gt.shape[0]):
                #    if ((kpt_gt[i, 0] >= 0) * 
                #            (kpt_gt[i, 0] < cfg.input_body_shape[1]) *
                #            (kpt_gt[i, 1] >= 0) * 
                #            (kpt_gt[i, 1] < cfg.input_body_shape[0])):
                #        #pdb.set_trace()
                #        cv2.circle(img_input, (int(kpt_gt[i, 0]), int(kpt_gt[i, 1])),  radius=1, color=(255, 0, 255), thickness=-1, lineType=cv2.LINE_AA)

                pdb.set_trace()
                img_input = vis_keypoints_error(img_input, joint_proj, kpt_gt)

                cv2.imwrite(f'kpt_input.jpg', img_input[:, :, ::-1])
                ...
                img_input = np.ascontiguousarray(body_img[-1, ...].permute([1,2,0]).detach().cpu().numpy() * 255, dtype=np.uint8)
                img_input = render_mesh(img_input, mesh_cam[-1, ...].cpu().detach().numpy(), self.smpl.face, {'focal': focal, 'princpt': princpt})
                for i in range(joint_proj.shape[1]):
                    if ((joint_proj[-1, i, 0] >= 0) * 
                            (joint_proj[-1, i, 0] < cfg.input_body_shape[1]) *
                            (joint_proj[-1, i, 1] >= 0) * 
                            (joint_proj[-1, i, 1] < cfg.input_body_shape[0])):
                        cv2.circle(img_input, (int(joint_proj[-1, i, 0]), int(joint_proj[-1, i, 1])),  radius=1, color=(0, 0, 255), thickness=-1, lineType=cv2.LINE_AA)

                cv2.imwrite(f'kpt_inputr.jpg', img_input[:, :, ::-1])
                #pdb.set_trace()
