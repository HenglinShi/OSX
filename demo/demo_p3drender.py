...
for num, indice in enumerate(indices):
    ...
    vis_mesh2 = render_mesh(vis_mesh2, mesh.cpu().numpy(), smpl.face, {'focal': focal, 'princpt': princpt})

    joint_proj = out['smplx_joint_proj'].detach().cpu().numpy()[0]
    joint_proj[:, 0] = joint_proj[:, 0] / cfg.input_body_shape[1] * cfg.input_img_shape[1]
    joint_proj[:, 1] = joint_proj[:, 1] / cfg.input_body_shape[0] * cfg.input_img_shape[0]
