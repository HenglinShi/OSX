...
def soft_argmax_2d(heatmap2d):
    ...
    accu_x = accu_x * torch.arange(width).to(accu_x.device).float()[None, None, :]
    accu_y = accu_y * torch.arange(height).to(accu_y.device).float()[None, None, :]
...
def soft_argmax_3d(heatmap3d):
    ...
    accu_x = accu_x * torch.arange(width).to(accu_x.device).float()[None, None, :]
    accu_y = accu_y * torch.arange(height).to(accu_x.device).float()[None, None, :]
    accu_z = accu_z * torch.arange(depth).to(accu_x.device).float()[None, None, :]
